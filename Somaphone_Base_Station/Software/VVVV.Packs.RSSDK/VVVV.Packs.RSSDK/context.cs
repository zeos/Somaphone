...
namespace VVVV.Nodes.RSSDK
{
    ...
    public class RSSDKContextNode : IPluginEvaluate, IPartImportsSatisfiedNotification, IPluginConnections
    {
        ...
        [Input("AllowProfileChange", IsSingle = true)]
        public IDiffSpread<bool> FAllowProfileChangeIn;
       ...
       //-- OUTPUT
        ...
        public void Evaluate(int SpreadMax)
        {
    		...
    		{
        		if(FMirror.IsChanged) SenseSession.device.SetMirrorMode((FMirror[0]) ? PXCMCapture.Device.MirrorMode.MIRROR_MODE_HORIZONTAL  : PXCMCapture.Device.MirrorMode.MIRROR_MODE_DISABLED);
	    		
	        	if(FEnableHand.IsChanged && FEnableHand[0]) SenseSession.sm.EnableHand("zeos");
	        	
	        	if(FEnableBlob.IsChanged && FEnableBlob[0]) SenseSession.sm.EnableBlob();
    	...
    	internal string SetupStreams()
    	{
			...
			//COLOR    		
			if( (FStreamColorMode != null) && (FStreamColorMode[0].Index > 0))
            {
            	...
            	if(EnableCaptureStream(PXCMCapture.StreamType.STREAM_TYPE_COLOR, profileInfo))
            	{
					...
					Status += "Enabled stream type " + FStreamColorMode[0].Name + "\n\r";
            	} 
            } 
            
            //DEPTH
			if( (FStreamDepthMode != null) && (FStreamDepthMode[0].Index > 0))
			{
            	...
            	if(EnableCaptureStream(PXCMCapture.StreamType.STREAM_TYPE_DEPTH, profileInfo))
            	{
					...
					Status += "Enabled stream type " + FStreamDepthMode[0].Name + "\n\r";
            	}
			}
			
			//IR
			if( (FStreamIRMode != null) && (FStreamIRMode[0].Index > 0))	
			{
            	...
            	if(EnableCaptureStream(PXCMCapture.StreamType.STREAM_TYPE_IR, profileInfo))
            	{
					...
					Status += "Enabled stream type " + FStreamIRMode[0].Name + "\n\r";
            	}
			}
